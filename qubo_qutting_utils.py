        ...
        for file in exp_data_file:
            try:
                ...
                for i, row in enumerate(input_df.itertuples(index=False), 1):
                    ...
                    for pm_ in partition_methods:
                        try:
                            ...
                            # Optionally, save the DataFrame
                            current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M")
                            filename = f'exp_' + str(n) + '_' + str(p) + '_' + str(seed1) + str(
                                seed) + '_cktCut_upToFailure_' + current_time + '.csv'
                ...
                if not df_failed.empty:
                    failed_filename = f'FailedExp_' + str(n) + '_' + str(p) + '_' + str(seed1) + '_' + str(
                        seed) + '_cktCut_' + current_time + '.csv'
